...
func TestMLFQSparseScheduling(t *testing.T) {
	inputs := []*Job{
		NewJob(1, 0, []JobInput{IOInstruction}),
		NewJob(2, 10, []JobInput{CPUInstruction, IOInstruction, CPUInstruction}),
	}
	...
	if len(expectedSystemBehaviour) != len(out) {
		t.Errorf("Mismatch in number of CPU actions, expected:%v, got:%v", expectedSystemBehaviour, out)
		return
	}
